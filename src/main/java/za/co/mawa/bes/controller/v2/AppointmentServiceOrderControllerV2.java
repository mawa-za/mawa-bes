...
import za.co.mawa.bes.dto.v2.appointment.serviceorder.AppointmentServiceOrderRequest;
...
public class AppointmentServiceOrderControllerV2 {
    ...
    public AppointmentServiceOrderControllerV2(
    ...
    public ResponseEntity<?> search(
        ...
        try {
            return ResponseEntity.ok(serviceOrderService.search(status, customerId, appointmentId, fromDate, toDate));
        } catch (Exception exception) {
            return badRequest(exception);
        }
    }
    ...
    public ResponseEntity<?> update(
            ...
            @RequestBody AppointmentServiceOrderRequest request,
            @RequestHeader(value = "X-User-Id", required = false) String currentUser
    ) {
        try {
            return ResponseEntity.ok(serviceOrderService.update(id, request, currentUser));
        } catch (Exception exception) {
            return badRequest(exception);
        }
    }

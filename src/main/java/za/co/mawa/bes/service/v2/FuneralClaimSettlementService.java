...
public class FuneralClaimSettlementService {
 ...
 private String resolveProviderPartnerName(String tenant,String id){if(id==null)return "Funeral service provider";List<String> n=jdbc.query("SELECT TRIM(CONCAT(COALESCE(first_name,''),' ',COALESCE(last_name,''))) FROM "+qualified(tenant,"partner")+" WHERE id=?",(rs,i)->rs.getString(1),id);return n.isEmpty()||n.get(0)==null||n.get(0).isBlank()?"Funeral service provider":n.get(0);}
 ...
 private void ensureProviderInvoice(String tenant,String serviceId,String claimId,Map<String,Object> claim){
  ...
  try {
   Map<String,Object> h=jdbc.queryForMap("SELECT TRIM(CONCAT(COALESCE(p.first_name,''),' ',COALESCE(p.last_name,''))) holder_name,p.identity_number FROM membership_claim c JOIN membership m ON m.id=c.membership_id JOIN partner p ON p.id=m.partner_id WHERE c.id=?",claimId);
   holderName=Objects.toString(h.get("holder_name"),null);holderIdentity=Objects.toString(h.get("identity_number"),null);

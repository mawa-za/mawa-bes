...
SELECT DISTINCT
        ...
        (SELECT number FROM transaction WHERE transaction.id = payment_batch_link.transaction1 ) AS 'batch_number',
...
LEFT JOIN transaction_link AS payment_batch_link  ON transaction.id = payment_batch_link.transaction2
	AND (payment_batch_link.type = 'PAYMENT-BATCH-LINK')
